...
from denser.models.splatfacto import SplatfactoModel, SplatfactoModelConfig
from denser.data.utils.objects import Annotations, BoundingBox


@dataclass
class SceneModelConfig(SplatfactoModelConfig):
    """Gaussian Splatting Model Config"""

    _target: Type = field(default_factory=lambda: SceneModel)
...
class SceneModel(SplatfactoModel):
    ...
    def populate_modules(self):
        ...
        self.object_annotations: Annotations= self.get_annotations(self.kwargs["metadata"])
        
        for idx, (obj_id, obj_meta) in enumerate(self.object_annotations.objects_meta.items()):
      
            self.all_models[self.get_object_model_name(obj_id)] = self.get_object_model(obj_meta, obj_id, idx)

        self.visible_model_names: List[str] = list(self.all_models.keys())
        self.means = torch.cat([self.background_model.means], dim=0)
        

    def get_object_model(self,obj_meta, obj_id, idx):
        object_model_config = copy.deepcopy(self.config.object_model_template)
        object_model_config.sh_degree = self.config.sh_degree
        object_model_config.extent = torch.from_numpy(obj_meta.size).float() 
        return object_model_config.setup(
                scene_box=self.scene_box,
                num_train_data=self.num_train_data,
                seed_points=self.object_annotations.get_seed_pts(obj_id),
                metadata = self.kwargs["metadata"],
                model_idx_in_scene_graph = idx + 1
            )

    def get_annotations(self,metadata):
        return metadata.get("annotations", Annotations(annotations=None))
    ...
    def build_frame_idx_map(self):
        frame_idx_map = {}
        for frame_idx, timestamp in enumerate(self.object_annotations.annos.keys()):
    ...
    def get_param_groups(self) -> Dict[str, List[Parameter]]:
        ...
        for model_name, model in self.all_models.items():
            for group_name, params in model.get_param_groups().items():
                if group_name in groups:
                    continue
                else:
                    groups[group_name] = params
    ...
    def get_wavelet_features(self, frame, trackId, obj_model: SplatfactoModel):
        frame_list = self.object_annotations.objects_frames[str(trackId)]
        if len(frame_list) == 1:
            normalized_frame = 1.0
        else:
            normalized_frame = (frame - frame_list[0]) / (frame_list[-1] - frame_list[0])
        t = normalized_frame * obj_model.config.time_scale_factor
        wavelet_base = compute_wavelet_features(t, obj_model.config.wavelets_features_dim, 
                                                    scale=obj_model.scale_param,
                                                    translation=obj_model.translation_param).to(self.device)
        
        return torch.sum(obj_model.features_dc * wavelet_base[..., None], dim=1, keepdim=True)
    ...
    def super_outputs(self, camera: Cameras):
    ...
    def get_outputs(self, camera: Cameras) -> Dict[str, Union[torch.Tensor, List]]:
        ...
        annos_t: List[BoundingBox] = self.object_annotations[str(int(camera.times.item()))] 
        ...
        if annos_t is not None and len(annos_t):
            for anno in annos_t:
                ...
                if obj_model.num_points == 0:
                    continue
                if self.config.wavelets_features_dim > 1:
                    object_features_dc.append(self.get_wavelet_features(anno.frame, trackId, obj_model))
                else:
...
def ricker_wavelet(t, a,translation):
    """
    Mexican Hat (Ricker) wavelet function.
    t: time or spatial variable
    a: scale parameter
    translation: translation parameter
    """
    factor = 2 / (math.sqrt(3 * abs(a)) * (math.pi ** 0.25))
    return factor * (1 - ((t - translation) / a) ** 2) * torch.exp(- ((t - translation) ** 2) / (2 * a ** 2))

def compute_wavelet_features(time, dim, scale,translation):
    """
    Computes Mexican Hat wavelet features for a given time signal.
    time: normalized time variable
    dim: dimension of the wavelet features
    scale: scale parameter for the wavelet
    translation: translation parameter
    """
    if isinstance(time, float):
        time = torch.tensor(time).to(scale.device)
    t = time.view(-1, 1)
    wavelet_features = torch.zeros(t.shape[0], dim, dtype=t.dtype, device=t.device)
    for i in range(dim):
        wavelet_features[:, i] = ricker_wavelet(t, scale[i] * (i + 1),translation[i] * i)
